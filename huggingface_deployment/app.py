...
def engineer_features(workout: Dict) -> np.ndarray:
    ...
    Returns: Array of shape [seq_len, 14]
    """
    ...
    # Additional temporal features (V2)
    features[:, 11] = create_lag_feature(speed, lag=10)  # Medium-term lag
    features[:, 12] = compute_rolling_mean(speed, window=5)  # Short-term smoothing
    features[:, 13] = create_lag_feature(altitude, lag=60)  # Long-term elevation lag
...
def main():
    ...
    if checkpoint:
        ...
        st.sidebar.markdown("""
        **Model**: 2-layer LSTM  
        **Input Features**: 14 (speed, altitude, temporal)  
        **Target**: Sub-3-hour marathon training  
        **Performance**: 7.42 BPM MAE ✨
        """)
